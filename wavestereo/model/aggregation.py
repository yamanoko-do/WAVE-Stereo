...
# Modified for AnLIGStereoPP: Add sigmoid to attention for FP16 safety
import torch
...
class Aggregation(nn.Module):
    ...
    def forward(self, x, features_left):
        """
        AnLIGStereoPP: Removed FP32 custom_fwd - safe for FP16 now!
        Attention is normalized with sigmoid to prevent overflow.
        """
        x = self.conv0(x)
        if self.left_att:
        ...
        conv5 = F.relu(self.conv5(conv4) + self.redir2(conv2), inplace=True) #1/8
        conv6 = F.relu(self.conv6(conv5) + self.redir1(x), inplace=True) #1/4
...
class MobileV2Residual(nn.Module):
    """
    倒残差块，扩张和收缩操作允许在不同视差传播信息,深度卷积在空间域传播信息
    expanse_ratio:channel扩张的维度
    """
...
class AttentionModule(nn.Module):
    """
    MSCA - Modified for AnLIGStereoPP
    Adds sigmoid normalization to attention to prevent FP16 overflow
    """
    ...
    def forward(self, cost, x):
        ...
        # AnLIGStereoPP: Normalize attention with sigmoid to [0, 1]
        # This prevents attention*cost from overflowing FP16
